 higher-order component (HOC) is an advanced technique in React for reusing component logic. HOCs are not part of the React API. They are a pattern that emerges from React’s compositional nature
...
In other words, `connect` is a [[higher-order-function]] that returns a [[higher-order-components]]!
